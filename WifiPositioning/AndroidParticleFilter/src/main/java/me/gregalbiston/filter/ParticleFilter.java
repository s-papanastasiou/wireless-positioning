...
import me.gregalbiston.androidparticlefilter.InertialPoint;
...
public class ParticleFilter {
    ...
    private final static int PARTICLE_MAX = 25;

    public static Cloud filter(Cloud cloud, Point posProba, InertialPoint inertialPoint) {

        // Weight calculus
        List<Particle> weightList = weightCloud(cloud.getParticles(), posProba);
        ...
        List<Particle> newRandomCloudList = newRandomCloud(moveCloudList);
    ...
    private static List<Particle> weightCloud(List<Particle> particles, Point posProba) {
    ...
    private static List<Particle> newRandomCloud(List<Particle> particles) {

        List<Particle> newParticles = new ArrayList<>();
        for (Particle particle : particles) {

            double dx = 0.3 * Math.sqrt(-Math.log(1 - Math.random()));
            double dy = 0.3 * Math.sqrt(-Math.log(1 - Math.random()));
